var config = require('../../config/config.js');
var co = require('co');
var parse = require('co-body');
var busboy = require('co-busboy');
var thunkify = require('thunkify');
var views = require('co-views');
var render = views('views', {
    default: 'jade'
});
var fs = require('fs');
var path = require('path');

var PModel = require('../../proxy/catalog.js');
var ProductModel = require('../../proxy/product.js');

function routes(app) {

    app.get('/admin/:catalogId/product', defaultIndex);

    app.post('/admin/prodrctdelmulti', delmulti);
    app.post('/admin/:catalogId/product/upload', upload);
    app.get('/admin/:catalogId/product/pagerinfo', pagerInfo);
    app.get('/admin/:catalogId/product/pagerlist', pagerlist);
    app.get('/admin/catalog/detail', detail);
    app.post('/admin/catalog/update', update);
    app.post('/admin/catalog/del', del);
    console.log('delmulti');

}
var getCatalogID = function(url) {
    var catalogId = /^\/admin\/(.*?)\//.exec(url)[1];
    return catalogId;
}
var defaultIndex = function * () {
    var catalogId = getCatalogID(this.url);
    var html = yield render('/admin/product', {
        title: '图片管理',
        pageId: catalogId
    });
    this.body = html;
};

//上传图片
var upload = function * () {
    var catalogId = getCatalogID(this.url);
    var parts = busboy(this);
    var part;
    while (part = yield parts) {
        console.log(part);
        if (part.filename) {
            var fpc = thunkify(ProductModel.create);
            var obj = {
                name: part.filename,
                catalog: catalogId,
                type: 1,
                status: 1,
                desc: ''
            };
            var reuslt = yield fpc(obj);
            var vpath = config.getImagePath(reuslt._id + '.jpg');
            var stream = fs.createWriteStream(vpath);
            part.pipe(stream);
            console.log('uploading %s -> %s', part.filename, stream.path);
        }
    }
    this.body = catalogId;
};
//分页信息
var pagerInfo = function * () {
    var catalogId = getCatalogID(this.url);
    var pagerInfo = thunkify(ProductModel.pagerInfo);
    var onePageCount = config['list_one_page_count'];
    try {
        var obj = yield pagerInfo(catalogId);
        this.body = {
            code: 0,
            msg: 0,
            result: {
                count: obj.count,
                pageCount: Math.ceil(obj.count * 1.0 / onePageCount),
                onePageItemCount: onePageCount
            }
        };
    } catch (e) {
        this.body = {
            code: 0,
            msg: ''
        };
    } finally {}
}
//pagerlist
var pagerlist = function * () {
    var catalogId = getCatalogID(this.url);
    var pageIndex = this.request.query.pageIndex;
    var pageSize = this.request.query.pageSize;
    var onePageCount = config['list_one_page_count'];
    var cfun = thunkify(ProductModel.list);
    try {
        var result = yield cfun(catalogId, pageIndex, pageSize);
        result.forEach(function(elem) {
            elem.imageurl = path.join(config.upload_url, elem._id + '.jpg');
        });

        this.body = {
            code: 0,
            msg: '',
            result: result
        };
    } catch (e) {
        this.body = {
            code: 1,
            msg: e.message
        };
    } finally {}
}

//delmulti
var delmulti = function * () {
    var body = yield parse(this, {
        limit: '1kb'
    });
    var detailIDs = body.detailIDs.split(',');
    var cfun = thunkify(ProductModel.deleteByIds);
    try {
        var result = yield cfun(detailIDs);
        detailIDs.forEach(function(ele){
          var vpath = config.getImagePath(ele + '.jpg');
          fs.unlinkSync(vpath);
        });
        this.body = {
            code: 0,
            msg: ''
        };
    } catch (e) {
        this.body = {
            code: 1,
            msg: e.message
        };
    } finally {}
}


//detail
var detail = function * () {
    var _id = this.request.query._id;
    var cfun = thunkify(PModel.getById);
    try {
        var result = yield cfun(_id);
        this.body = {
            code: 0,
            msg: '',
            result: result
        };
    } catch (variable) {
        this.body = {
            code: 1,
            msg: e.message
        };
    } finally {}
}
//－－－－－－－－－－－－－－－－－－－－－－－－－－－－－update
var update = function * () {
    var body = yield parse(this, {
        limit: '1kb'
    });
    var _id = body._id;
    var name = body.name;
    var code = body.code;
    var type = parseInt(body.type) || 1;
    var status = parseInt(body.status) || 1;
    var desc = body.desc;
    var objItem = {
        _id: _id,
        name: name,
        code: code,
        type: type,
        status: status,
        desc: desc
    };

    if (!_id.length) { //update
        var cfun = thunkify(PModel.create);
        try {
            var result = yield cfun(objItem);
            this.body = {
                code: 0,
                msg: '',
                result: result
            };
        } catch (e) {
            this.body = {
                code: 1,
                msg: e.message
            };
        } finally {}
    } else { //new
        var cfun = thunkify(PModel.update);
        try {

            var result = yield cfun(objItem);
            this.body = {
                code: 0,
                msg: '',
                result: result
            };
        } catch (e) {
            this.body = {
                code: 1,
                msg: e.message
            };
        } finally {}
    }
}
//del
var del = function * () {
    var body = yield parse(this, {
        limit: '1kb'
    });
    var _id = body._id;
    var cfun = thunkify(PModel.deleteById);
    try {
        var result = yield cfun(_id);
        //del file
        var vpath = config.getImagePath(_id + '.jpg');
        fs.unlinkSync(vpath);
        this.body = {
            code: 0,
            msg: ''
        };
    } catch (e) {
        this.body = {
            code: 1,
            msg: e.message
        };
    } finally {}
}




module.exports = routes;
